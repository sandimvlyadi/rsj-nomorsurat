...
$(document).ready(function(){
	...
	table = $('#dataTable').DataTable({
        ...
        'ajax' : {
        	'url'	: baseurl + 'nomor-sk/datatable/',
            'type'	: 'GET',
            'data'  : function(d){
    ...
    $.ajax({
        type: 'GET',
        url: baseurl + 'nomor-sk/select-bagian/',
        dataType: 'json',
        success: function(response){
            if(response.result){
...
$('select[name="id_bagian_surat"], select[name="id_ujung_surat"], input[name="tanggal"]').on('change', function(){
    ...
    if (id_bagian_surat != 0 && id_ujung_surat != 0 && tanggal != '') {
        $.ajax({
            type: 'POST',
            url: baseurl + 'nomor-sk/get-nomor/',
            data: {
