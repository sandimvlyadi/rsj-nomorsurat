...
class Nomor_sk_model extends CI_Model {
	...
	function _get($data = array())
    {
        ...
        if ($data['search']['value'] && !isset($data['all'])) {
            ...
            $q .= "WHERE (b.`nama_jenis_surat` LIKE '%". $s ."%' OR c.`nama_bagian_surat` LIKE '%". $s ."%' OR d.`display_name` LIKE '%". $s ."%' OR a.`nomor` LIKE '%". $s ."%' OR a.`tujuan` LIKE '%". $s ."%' OR a.`perihal` LIKE '%". $s ."%' OR a.`tanggal` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL AND a.`id_jenis_surat` = 1 ";
        } else{
        	$q .= "WHERE a.`deleted_at` IS NULL AND a.`id_jenis_surat` = 1 ";
        }
    ...
    function get_nomor($data = array())
    {
        ...
        $q = "SELECT * FROM `nomor_surat` WHERE `id_jenis_surat` = 1 AND `tanggal` LIKE '". $tanggal ."%';";
        $r = $this->db->query($q, false)->result_array();
        $n = count($r);
        $nomor = '';
        if ($n > 0) {
