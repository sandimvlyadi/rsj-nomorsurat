...
class Bagian_pengguna_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.*, b.`nama` AS `nama_level_pengguna` FROM `bagian_pengguna` a LEFT JOIN `level_pengguna` b ON a.`id_level_pengguna` = b.`id` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (b.`nama` LIKE '%". $s ."%' OR a.`nama` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
    ...
    function edit($id = 0)
    {
        $result = array(
            'result'    => false,
            'msg'       => 'Data bagian pengguna tidak ditemukan.'
        );
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `bagian_pengguna` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
    ...
    function select($id = 0)
    {
        ...
        $q = "";
        if ($id == 0) {
            $q = "SELECT * FROM `bagian_pengguna` WHERE `deleted_at` IS NULL;";
        } else{
            $q = "SELECT * FROM `bagian_pengguna` WHERE `id` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
    ...
    function select_by_id_level_pengguna($id = 0)
    {
        ...
        if ($id == 0) {
            ...
            $q = "SELECT * FROM `bagian_pengguna` WHERE `id_level_pengguna` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
