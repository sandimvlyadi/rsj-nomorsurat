...
class Ujung_surat_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.* FROM `ujung_surat` a ";
    ...
    function edit($id = 0)
    {
        $result = array(
            'result'    => false,
            'msg'       => 'Data ujung surat tidak ditemukan.'
        );
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `ujung_surat` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
    ...
    function select($id = 0)
    {
        ...
        $q = "";
        if ($id == 0) {
            $q = "SELECT * FROM `ujung_surat` WHERE `deleted_at` IS NULL;";
        } else{
            $q = "SELECT * FROM `ujung_surat` WHERE `id` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
