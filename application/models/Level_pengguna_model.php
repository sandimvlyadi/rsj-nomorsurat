...
class Level_pengguna_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.* FROM `level_pengguna` a ";
    ...
    function edit($id = 0)
    {
        $result = array(
            'result'    => false,
            'msg'       => 'Data level pengguna tidak ditemukan.'
        );
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `level_pengguna` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
