...
class Registrasi_pengguna_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.* FROM `registrasi_pengguna` a ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`nip` LIKE '%". $s ."%' OR a.`passowrd` LIKE '%". $s ."%' OR a.`email` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
