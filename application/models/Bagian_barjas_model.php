...
class Bagian_barjas_model extends CI_Model {
	...
	function _get($data = array())
    {
        ...
        if ($data['search']['value'] && !isset($data['all'])) {
            ...
            $q .= "WHERE (b.`nama` LIKE '%". $s ."%' OR a.`kode` LIKE '%". $s ."%' OR a.`nama` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL AND a.`id_jenis_surat` = 4 ";
        } else{
